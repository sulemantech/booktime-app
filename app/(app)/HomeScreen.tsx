...
// Dummy data for different book sections
const bookData = [
...
/**
 * Renders a single book item within a horizontal list.
 * @param {object} props - The component props.
 * @param {string} props.title - The title of the book.
 * @param {string} props.author - The author of the book.
 * @param {string} props.image - The URI for the book cover image.
 * @param {object} props.router - The router object from useRouter hook.
 */
const BookItem = ({ title, author, image, router }) => (
  <TouchableOpacity
    style={styles.bookItem}
    onPress={() => router.push({ pathname: 'BookDetailScreen', params: { bookTitle: title, bookImage: image } })}
  >
...
/**
 * Renders a section header with a title and a "view all" link.
 * @param {object} props - The component props.
 * @param {string} props.title - The title of the section.
 * @param {object} props.router - The router object from useRouter hook.
 */
const SectionHeader = ({ title, router }) => (
...
/**
 * Renders an icon in the header.
 * @param {object} props - The component props.
 * @param {string} props.source - The icon emoji or text.
 ...
 * It displays different sections of books.
 */
const HomeScreen = () => {
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 20 }}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerRight}>
            <HeaderIcon source="👑" />
            <HeaderIcon source="🌐" />
            <HeaderIcon source="🔍" onPress={() => router.push('SearchScreen')} />
          </View>
        </View>

        {/* Sections */}
        <SectionHeader title="My Stories" router={router} />
        <FlatList
          data={bookData}
          renderItem={({ item }) => <BookItem {...item} router={router} />}
        ...
        <SectionHeader title="Continue Reading" router={router} />
        <FlatList
          data={miaData}
          renderItem={({ item }) => <BookItem {...item} router={router} />}
          keyExtractor={(item) => item.id}
        ...
        <SectionHeader title="Read Again" router={router} />
        <FlatList
          data={tenInBedData}
          renderItem={({ item }) => <BookItem {...item} router={router} />}
          keyExtractor={(item) => item.id}
        ...
        <SectionHeader title="Friends and Feelings" router={router} />
        <FlatList
          data={miaData}
          renderItem={({ item }) => <BookItem {...item} router={router} />}
          keyExtractor={(item) => item.id}
        ...
        <SectionHeader title="We Can Count" router={router} />
        <FlatList
          data={tenInBedData}
          renderItem={({ item }) => <BookItem {...item} router={router} />}
          keyExtractor={(item) => item.id}
